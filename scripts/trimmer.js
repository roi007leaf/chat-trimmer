...
export class ChatTrimmer {
  ...
  async trim(messages = null, options = {}) {
    ...
    try {
      ...
      const enableArchiving = game.settings.get(
        "chat-trimmer",
        "enableArchiving",
      );
      ...
      if (!enableArchiving) {
        console.log(
          "Chat Trimmer | Archiving disabled - deleting messages without archiving",
        );
        ...
        ui.notifications.info(
          game.i18n.format(
            "CHATTRIMMER.Notifications.TrimmingCompleteNoArchive",
            {
  ...
  isKeyEvent(msg, contentLower = null) {
    ...
    // 5. High-level spells (4th level and above) - but only actual spell casts, not just spell cards
    if (hasRoll && msg.flags?.pf2e?.origin?.type === "spell") {
    ...
    // Also check content+flavor for high-level spell indicators - only for actual rolls
    const spellLevelMatch = combined.match(
      /\b(\d+)(?:st|nd|rd|th)[-\s]?level spell/i,
    );
    if (hasRoll && spellLevelMatch && parseInt(spellLevelMatch[1]) >= 4) {
      return true;
    }
    ...
    // 8. Only truly severe conditions (not common combat conditions)
    // Note: dying, death, unconscious are already handled in section 2
    if (
      combined.includes("doomed") ||
      combined.includes("paralyzed") ||
      combined.includes("petrified")
    ) {
      console.log("Chat Trimmer | Key Event detected (severe condition):", {
        text: combined.substring(0, 100),
        speaker: msg.speaker?.alias,
      });
      return true;
  ...
  formatIndividualDisplay(msg) {
    ...
    if (
      ...
      // Extract weapon/action name - try multiple paths
      // First try the origin paths
      let actionName =
        origin?.item?.name ||
        origin?.name ||
        msg.flags.pf2e.context?.item?.name;

      // If not found, try extracting from flavor text (e.g., "Melee Strike: Longsword" -> "Longsword Strike")
      if (!actionName && msg.flavor) {
        // Strip HTML tags first to avoid parsing issues
        const cleanFlavor = MessageParser.stripHTML(msg.flavor);
        // Match text after the last colon (more flexible)
        const flavorMatch = cleanFlavor.match(/:\s*([^:]+)\s*$/);
        if (flavorMatch) {
          const extractedName = flavorMatch[1].trim();
          // Only use if it's not empty and doesn't look like a full sentence
          if (
            extractedName &&
            ...
            actionName = extractedName.includes("Strike")
              ? extractedName
              : `${extractedName} Strike`;
          }
        }
      }

      // Final fallback
      actionName = actionName || "Strike";
      ...
      return `⚔️ ${speaker}: ${actionName}${targetSuffix}${outcomeText}${rollTotal}`;
    }
    ...
    if (msg.flags?.pf2e?.origin?.type === "spell") {
      ...
      const speaker =
        msg.speaker?.alias || msg.speaker?.actor || msg.user?.name || "Unknown";
      return `✨ ${speaker}: ${spellName}${targetSuffix}`;
    }
    ...
    if (MessageParser.isRoll(msg)) {
      ...
      const speaker =
        msg.speaker?.alias || msg.speaker?.actor || msg.user?.name || "Unknown";
      return `🎲 ${speaker}: ${rollSummary}${targetSuffix}`;
    }
    ...
    if (checkMatch) {
      const skillName =
        checkMatch[1].charAt(0).toUpperCase() + checkMatch[1].slice(1);
  ...
  determineMessageCategories(msg) {
    ...
    const isTrade =
      ...
      content.includes("sell item") ||
      content.includes("buy item") ||
      content.includes(" to merchant") ||
      content.includes(" from merchant");
  ...
  calculateStatistics(entries, detected) {
    ...
    for (const entry of entries) {
      ...
      if (
        ...
        MessageParser.isRoll(entry.originalMessage)
      ) {
        stats.totalRolls++;
        ...
        console.log("Chat Trimmer | Stats: Checking roll message:", {
          ...
          flavor: entry.originalMessage.flavor
            ? entry.originalMessage.flavor.substring(0, 50)
            : "no flavor",
        });
        ...
        if (outcome === "criticalSuccess") {
          ...
          console.log(
            "Chat Trimmer | Stats: Critical success found (PF2e flag)!",
          );
        } else if (outcome === "criticalFailure") {
          stats.criticalFails++;
          console.log(
            "Chat Trimmer | Stats: Critical failure found (PF2e flag)!",
          );
        } else {
          ...
          if (
            ...
            stats.criticalSuccesses++;
            console.log(
              "Chat Trimmer | Stats: Critical success found (content/flavor check)!",
              {
            ...
            stats.criticalFails++;
            console.log(
              "Chat Trimmer | Stats: Critical failure found (content/flavor check)!",
            );
          }
        }
      }
