    ...
    if (this.chatMessage) {
      ...
      if (messageElement) {
        ...
        const slideDown = (el, duration = 200) => {
          ...
          el.style.display = "";
          const targetDisplay = window.getComputedStyle(el).display;
          if (targetDisplay === "none") el.style.display = "block";
        ...
        if (game.system.id === "pf2e") {
          for (const actionEl of messageElement.querySelectorAll(
            "[data-action]",
          )) {
            actionEl.addEventListener("click", async (event) => {
              ...
              console.log(
                `Archive Viewer | Clicked element <${button.tagName}> with action: ${action}`,
              );
              ...
              if (!actor || !item) {
                console.warn(
                  "Archive Viewer | Could not resolve actor or item",
                );
                return;
              }
              ...
              try {
                ...
                switch (action) {
                  ...
                  case "critical": {
                    console.log(
                      "Archive Viewer | Calling strike damage/critical",
                    );
                    const isCritical =
                    ...
                    if (
                      ...
                      const strike =
                        ...
                        strikes.find(
                          (s) =>
                            strikeIdentifier &&
                            s.identifier === strikeIdentifier,
                        ) ||
                        strikes.find(
                          (s) => s.item?.id === item.id || s.slug === item.slug,
                        );

                      if (strike) {
                        console.log(
                          `Archive Viewer | Executing PF2e strike ${isCritical ? "critical" : "damage"}`,
                        );
                        ...
                        await strike.damage?.({
                          ...
                          getFormula: isCritical
                            ? (d) => d.criticalFormula
                            : undefined,
                        });
                      } else {
                        console.warn(
                          "Archive Viewer | Could not find strike action, falling back to item.rollDamage",
                        );
                        await item.rollDamage?.({
                          event,
                          message: this.chatMessage,
                  ...
                  case "attack":
                    ...
                    if (
                      ...
                      if (strike && strike.variants?.[variantIndex]) {
                        console.log(
                          "Archive Viewer | Executing PF2e strike attack",
                        );
                  ...
                  case "target-applyDamage": {
                    console.log(
                      "Archive Viewer | Applying damage/healing from archived message",
                    );
                    const multiplier = Number(
                      dataset.multiplier ||
                        (action === "apply-healing" || action === "applyHealing"
                          ? -1
                          : 1),
                    );
                    try {
                      ...
                      if (
                        toolbelt?.active &&
                        toolbeltApi?.target?.applyDamage
                      ) {
                        console.log(
                          "Archive Viewer | Using PF2e Toolbelt for damage application",
                        );
                        await toolbeltApi.target.applyDamage(
                          this.chatMessage,
                          multiplier,
                        );
                        break;
                      ...
                      if (typeof applyDamageFn === "function") {
                        ...
                        if (isTargetAction) {
                          const targetInfo =
                            context?.target ||
                            this.chatMessage.flags?.pf2e?.target;
                          if (targetInfo?.token) {
                            const tokenUuid = targetInfo.token.startsWith(
                              "Scene.",
                            )
                              ? targetInfo.token
                        ...
                        if (tokens.length === 0) {
                          ui.notifications.warn(
                            "PF2E.ErrorMessage.NoTokenSelected",
                            { localize: true },
                          );
                        } else {
                          const roll =
                            this.chatMessage.rolls.find(
                              (r) => r.constructor.name === "DamageRoll",
                            ) || this.chatMessage.rolls[0];
                          if (!roll)
                            throw new Error("No damage roll found in message");
                      ...
                      console.error(
                        "Archive Viewer | Error applying damage:",
                        err,
                      );
                      ui.notifications.error(
                        "Could not apply damage from archived message",
                      );
                    }
                    break;
                  }
                  ...
                  case "target-shieldBlock":
                    console.log(
                      "Archive Viewer | Applying shield block from archived message",
                    );
                    try {
                      // Log available PF2e API structure for debugging
                      console.log("Archive Viewer | PF2e API structure:", {
                        "game.pf2e": game.pf2e
                          ? Object.keys(game.pf2e)
                          : "undefined",
                        "game.pf2e.actions": game.pf2e?.actions
                          ? Object.keys(game.pf2e.actions)
                          : "undefined",
                        "CONFIG.PF2E": CONFIG.PF2E
                          ? Object.keys(CONFIG.PF2E)
                          : "undefined",
                      });
                      ...
                      const shieldBlockFn =
                        ...
                        game.pf2e?.system?.chat?.ShieldBlock
                          ?.applyFromMessage ||
                        CONFIG.PF2E?.actions?.shieldBlock;

                      if (typeof shieldBlockFn === "function") {
                        console.log(
                          "Archive Viewer | Found PF2e shield block function, calling it",
                        );
                      ...
                      // 2. Try message's own handler
                        ...
                        console.log(
                          "Archive Viewer | Trying direct message flag approach",
                        );
                        const messageFlags = this.chatMessage.flags?.pf2e;
                        console.log(
                          "Archive Viewer | Message flags:",
                          messageFlags,
                        );

                        // Try to find the shield block action data
                        const shieldBlockData =
                          messageFlags?.context?.shieldBlock;
                        if (shieldBlockData) {
                          console.log(
                            "Archive Viewer | Found shield block data in flags:",
                            shieldBlockData,
                          );
                          ...
                          if (actorUuid) {
                            ...
                            if (
                              actor &&
                              typeof actor.applyShieldBlock === "function"
                            ) {
                              console.log(
                                "Archive Viewer | Using actor.applyShieldBlock",
                              );
                              await actor.applyShieldBlock(this.chatMessage);
                            } else {
                              console.warn(
                                "Archive Viewer | Actor found but no applyShieldBlock method",
                              );
                              ui.notifications.warn(
                                "Shield block not available. Please apply manually.",
                              );
                            }
                          } else {
                            console.warn(
                              "Archive Viewer | No actor UUID in message flags",
                            );
                            ui.notifications.warn(
                              "Cannot determine actor for shield block. Please apply manually.",
                            );
                          }
                        } else {
                          console.warn(
                            "Archive Viewer | No shield block data in message flags",
                          );
                          ui.notifications.warn(
                            "Shield block not available from archived message. Please apply manually.",
                          );
                        }
                      } else {
                        console.warn(
                          "Archive Viewer | No PF2e ShieldBlock API found",
                        );
                        console.warn(
                          "Archive Viewer | Available in game.pf2e:",
                          game.pf2e ? Object.keys(game.pf2e) : "undefined",
                        );
                        ui.notifications.warn(
                          "Shield block not supported from archived messages. Please apply shield damage manually.",
                        );
                      }
                    } catch (err) {
                      console.error(
                        "Archive Viewer | Error applying shield block:",
                        err,
                      );
                      ui.notifications.error(
                        `Shield block failed: ${err.message}`,
                      );
                    }
                    break;
                  ...
                  case "expandRoll": {
                    ...
                    if (tooltip) {
                      const wasHidden = isHidden(tooltip);
                      console.log(
                        `Archive Viewer | Tooltip found. Hidden? ${wasHidden}`,
                      );
                      ...
                      console.warn(
                        "Archive Viewer | No .dice-tooltip found to expand",
                      );
                    }
                    break;
                  }
                  ...
                  case "set-targets":
                    console.log(
                      'Archive Viewer | "Set Targets" action clicked (ignored in archive)',
                    );
                  ...
                  case "expand-damage-context": {
                    console.log(
                      "Archive Viewer | Toggling damage context visibility",
                    );
                    const button = event.currentTarget;

                    // Search strategy 1: Relative to message content
                    // PF2e typically puts it in .message-content -> .damage-application
                    let damageApplication = button
                      .closest(".message-content")
                      ?.querySelector(".damage-application");

                    // Search strategy 2: Relative to chat message root
                    if (!damageApplication) {
                      damageApplication = button
                        .closest(".chat-message")
                        ?.querySelector(".damage-application");
                    }

                    // Search strategy 3: sibling/parent search (handles different DOM structures)
                    if (!damageApplication) {
                      ...
                      damageApplication =
                        container?.querySelector(".damage-application") ||
                      ...
                      if (!damageApplication) {
                        damageApplication = this.element.querySelector(
                          ".damage-application",
                        );
                      }
                    }
                    ...
                    if (damageApplication) {
                      ...
                      const currentlyHidden = isHidden(damageApplication);

                      console.log(
                        `Archive Viewer | Found damage application container. Currently hidden: ${currentlyHidden}`,
                      );

                      if (currentlyHidden) {
                        damageApplication.classList.remove("hidden");
                        damageApplication.style.display = "flex";
                        button.setAttribute("aria-expanded", "true");
                        // Ensure buttons inside are active? (They should be captured by the main listener)
                      } else {
                        damageApplication.classList.add("hidden");
                        damageApplication.style.display = "none";
                        button.setAttribute("aria-expanded", "false");
                      }
                    } else {
                      console.warn(
                        "Archive Viewer | Could not find .damage-application container to toggle",
                      );
                      ...
                      ui.notifications.warn(
                        "Could not find damage buttons to expand.",
                      );
                    }
                    break;
                  }
                ...
                console.error(
                  "Archive Viewer | Error executing action:",
                  error,
                );
                ui.notifications.error(
          ...
          const handler = ui.chat._onChatCardAction?.bind(ui.chat);
          if (handler) {
            for (const btn of messageElement.querySelectorAll(
              "button[data-action]",
            )) {
        ...
        console.log(
          "Archive Viewer | Activated listeners for original message",
        );
      }
    }
  }
