...
export class ArchiveManager {
  ...
  async create(data) {
    ...
    const sessionNumber = game.settings.get(
      "chat-trimmer",
    ...
    if (existingArchive) {
      console.log(
        `Archive Manager | Found existing archive for session ${sessionNumber}, appending entries`,
      );
      return await this.appendToArchive(existingArchive, data);
    }

    console.log(
      `Archive Manager | Creating new archive for session ${sessionNumber}`,
    );
    const now = new Date();
    ...
    const storageType =
      game.settings.get("chat-trimmer", "storageType") || "journal";
    ...
    if (storageType === "external") {
      try {
        ...
        console.error(
          "Archive Manager | Failed to save to external file, falling back to journal:",
          err,
        );
        ui.notifications.warn(
          "Failed to save archive to file. Falling back to Journal storage.",
        );
  ...
  async appendToArchive(archive, data) {
    ...
    const newStats = {
      totalCombats:
        (existingStats.totalCombats || 0) + (data.stats?.totalCombats || 0),
      totalDialogues:
        (existingStats.totalDialogues || 0) + (data.stats?.totalDialogues || 0),
      totalSkillChecks:
        (existingStats.totalSkillChecks || 0) +
        (data.stats?.totalSkillChecks || 0),
      totalRolls:
        (existingStats.totalRolls || 0) + (data.stats?.totalRolls || 0),
      criticalSuccesses:
        (existingStats.criticalSuccesses || 0) +
        (data.stats?.criticalSuccesses || 0),
      criticalFails:
        (existingStats.criticalFails || 0) + (data.stats?.criticalFails || 0),
      itemsTransferred:
        (existingStats.itemsTransferred || 0) +
        (data.stats?.itemsTransferred || 0),
      xpAwarded: (existingStats.xpAwarded || 0) + (data.stats?.xpAwarded || 0),
    };

    // Merge search index
    const existingSearchIndex =
      archive.getFlag("chat-trimmer", "searchIndex") || {};
    const newSearchIndex = {
      keywords: [
        ...(existingSearchIndex.keywords || []),
        ...(data.searchIndex?.keywords || []),
      ],
      actors: [
        ...new Set([
          ...(existingSearchIndex.actors || []),
          ...(data.searchIndex?.actors || []),
        ]),
      ],
      scenes: [
        ...new Set([
          ...(existingSearchIndex.scenes || []),
          ...(data.searchIndex?.scenes || []),
        ]),
      ],
    };
    ...
    if (storageType === "external") {
      ...
      try {
        ...
        console.error(
          "Archive Manager | Failed to update external archive:",
          err,
        );
  ...
  async generateSessionSummary(archive, entriesParam = null) {
    ...
    const archiveDate = new Date(
      archive.getFlag("chat-trimmer", "archiveDate"),
    );
    const sessionStartTime = game.settings.get(
      "chat-trimmer",
    ...
    // Calculate duration
    const startTime =
      entries.length > 0 ? entries[0].timestamp : sessionStartTime;
    const endTime =
    ...
    const sortedParticipants = Array.from(participantsMap.values()).sort(
      (a, b) => {
        // Players first
    ...
    entries.forEach((entry) => {
      ...
      if (searchText.includes("critical")) {
        ...
        if (
          searchText.includes("critical fail") ||
          searchText.includes("critical miss") ||
          searchText.includes("fumble") ||
          searchText.includes("criticalfailure")
        ) {
          stats.criticalFails++;
        }
        // Then check for critical SUCCESSES
        else if (
          searchText.includes("critical success") ||
          searchText.includes("critical hit") ||
    ...
    console.log(
      `Archive Manager | Generating key events from ${entries.length} entries`,
    );
    if (entries.length > 0) {
    ...
    entries.forEach((entry) => {
      ...
      if (entry.isKeyEvent === true) {
        ...
        // Check if this is an actual roll (not just a spell/feat card sent to chat)
        const hasRoll =
          entry.originalMessage?.rolls &&
          entry.originalMessage.rolls.length > 0;

        // FILTER: Skip spell/feat/action cards and common conditions that were incorrectly marked as key events in old archives

        // Filter out "Current Conditions" status summary messages
        if (combinedText.includes("current conditions")) {
          console.log(
            "Archive Manager | Skipping Current Conditions summary:",
            displayText.substring(0, 100),
          );
          return; // Skip this entry
        }

        const originType = entry.originalMessage?.flags?.pf2e?.origin?.type;
        const isInfoCard =
          !hasRoll &&
          (originType === "spell" ||
            originType === "feat" ||
            originType === "action" ||
            (combinedText.includes("spell") && displayText.includes("✨")));

        // Filter out common/minor conditions (keep only severe ones like doomed, paralyzed, petrified)
        const isCommonCondition =
          (combinedText.includes("condition") ||
            combinedText.includes("persistent") ||
            combinedText.includes("slowed") ||
            combinedText.includes("stunned") ||
            combinedText.includes("frightened") ||
            combinedText.includes("enfeebled") ||
            combinedText.includes("clumsy") ||
            combinedText.includes("stupefied") ||
            combinedText.includes("drained") ||
            combinedText.includes("confused")) &&
          !combinedText.includes("doomed") &&
          !combinedText.includes("paralyzed") &&
          !combinedText.includes("petrified") &&
          !combinedText.includes("dying") &&
          !combinedText.includes("death") &&
          !combinedText.includes("unconscious");

        if (isInfoCard || isCommonCondition) {
          console.log(
            "Archive Manager | Skipping info card/common condition:",
            displayText.substring(0, 100),
          );
          return; // Skip this entry
        }
        ...
        if (
          hasRoll &&
          (combinedText.includes("critical success") ||
            combinedText.includes("critical hit"))
        ) {
          icon = "💥";
          importance = "high";
          ...
          hasRoll &&
          (combinedText.includes("critical fail") ||
            combinedText.includes("critical miss") ||
          ...
          icon = "🌟";
          importance = "high";
        } else if (hasRoll && combinedText.includes("spell")) {
          icon = "✨";
          importance = "medium";
        } else if (
          combinedText.includes("level") ||
          combinedText.includes("xp")
        ) {
          icon = "⭐";
          importance = "high";
        } else if (
      ...
      if (
        ...
        (lowerText.includes("damage") || lowerContent.includes("damage"))
      ) {
        ...
        if (targetActor && targetActor.system?.attributes?.hp) {
          ...
          if (currentHP <= 0) {
            ...
            if (msg.rolls && msg.rolls.length > 0) {
              const damageRoll = msg.rolls.find(
                (r) => r.constructor?.name === "DamageRoll" || r.formula,
              );
            ...
            if (damageAmount === null) {
              const damageMatch = content.match(
                /(\d+)\s*(?:points?\s*of\s*)?damage/i,
              );
              if (damageMatch) {
        ...
        const hpMatch =
          content.match(/HP:\s*0+\s*\/\s*\d+/i) ||
          content.match(/\b0\s*\/\s*\d+\s*HP/i);
        if (hpMatch && !targetActor) {
          ...
          if (
            ...
            const damageRoll = entry.originalMessage.rolls.find(
              (r) => r.constructor?.name === "DamageRoll" || r.formula,
            );
            if (damageRoll && damageRoll.total !== undefined) {
              damageAmount = Math.floor(damageRoll.total);
      ...
      // IMPORTANT: Only flag as key event if this is an actual roll, not just a description card
      else if (
      ...
      // Important items/loot
        ...
        (lowerText.includes("found") ||
          lowerText.includes("discovered") ||
          lowerText.includes("obtained")) &&
      ...
      // Only include actual spell casts with rolls, not spell cards
      else if (
    ...
    if (keyEvents.length > 0) {
      console.log(
        "Archive Manager | Sample key events:",
        keyEvents.slice(0, 3),
      );
    }
    ...
    return {
      ...
      compressionRatio:
        archive.getFlag("chat-trimmer", "compressionRatio") || 0,
      statistics: {
